...
import org.example.order_service.entity.primary.*;
...
public class DeliveryService {
    ...
    public Long createOrder(OrderDTO orderDTO, User user) throws InterruptedException {
        ...
        BlockingQueue<Long> orderId = new LinkedBlockingQueue<>();

        CompletableFuture.runAsync(() -> {
                jtaConfirmService.createOrder(order,orderId);
        });
        Long data = orderId.poll(200, TimeUnit.SECONDS);
        if (data != null) {
            return data;
        } else {
            throw new IllegalArgumentException("Ошибка сохранения Order");
        }
    }
